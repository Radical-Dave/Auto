locals {
  frontdoor_name = length(var.frontdoor_name) > 0 ? var.frontdoor_name : "${var.resource_group_name}-frontdoor"
  frontdoor_hostname = length(var.frontdoor_hostname) > 0 ? var.frontdoor_hostname : "${var.resource_group_name}.azurefd.net"
}
module "wafpolicy" {
  source = "../../templates/frontdoor-firewall-policy"
...
resource "azurerm_frontdoor" "this" {
  ...
  #location = var.frontdoor_location#deprecated - 3.10.2020>always 'Global'
  resource_group_name = var.resource_group_name
  enforce_backend_pools_certificate_name_check = var.frontdoor_certificate_enforced
  load_balancer_enabled = var.frontdoor_loadbalancer_enabled
  backend_pools_send_receive_timeout_seconds = var.frontdoor_backend_timeout
  tags = var.tags
  ...
  dynamic "routing_rule" {
    for_each = var.frontdoor_routing_rules
    content {
