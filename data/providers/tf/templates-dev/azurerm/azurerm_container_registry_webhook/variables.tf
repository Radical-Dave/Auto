...
variable "network_rule_set" { # change this to match actual objects
  ...
  type = object({
    default_action  = string                               #optional(string)
    ip_rule         = list(object({ ip_range = string }))  #optional(list(object({ip_range=string})))
    virtual_network = list(object({ subnet_id = string })) #optional(list(object({subnet_id=string})))
  })
  default = null
}
