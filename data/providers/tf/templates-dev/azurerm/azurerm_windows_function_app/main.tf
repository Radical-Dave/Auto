...
resource "azurerm_windows_function_app" "this" {
  ...
  tags = var.tags
  # dynamic "lifecycle" {
  #   for_each=var.lifecycle_ignores != null ? [true] : []
  #   content {
  #     ignore_changes=var.lifecycle_ignores
  #   }
  # }
  # lifecycle {
  #   ignore_changes=[
  #     app_settings["JOB_FUNCTION_BASE_URL"],
  #     app_settings["SCHEDULER_TIMER_TRIGGER"]
  #   ]
  # }  
  provisioner "local-exec" {
    command = var.source_control_repo ? join("", list("az functionapp deployment source config --ids ", azurerm_windows_function_app.this.id), " --repo-url $($source_control_repo)", " --branch $($source_control_branch) --manual-integration") : "true"
  }
}